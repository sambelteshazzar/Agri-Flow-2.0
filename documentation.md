...
The design system follows the "Earthen Organic" philosophy with a dual-mode theme (light: jade-tinted parchment `#EFFCF6` background; dark: deep forest `#041A14` background), three-tier typography hierarchy (DM Serif Display for the brand title, IBM Plex Sans for section headings, Inter for body text), a custom Tailwind color palette comprising jade (teal-green `#057855` accent, 50–950 scale), terra (sage-green neutral `#7BA896`, 50–900 scale reharmonized from warm brown to complement jade), and sunburst (warm gold `#D97706`, 50–900 scale), WCAG 2.2 AA accessibility compliance (4.5:1 contrast ratio), 44dp touch targets, focus-visible rings, reduced-motion support, and keyboard navigation. The palette migrated from an earlier soil/harvest/field scheme through a crimson iteration before settling on jade/terra/sunburst to achieve a cohesive teal-green and gold visual language rooted in West African agricultural landscapes.
