...
const Dashboard: React.FC = () => {
  ...
  const getRiskBg = (risk: string) => {
    switch(risk) {
      ...
      default: return 'bg-terra-50 dark:bg-[#163D2F] border-terra-200 dark:border-[#1E5A47]';
    }
  };
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            icon: AlertTriangle,
            iconBg: alerts.filter(a => a.severity === 'critical').length > 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-terra-50 dark:bg-[#163D2F]',
            iconColor: alerts.filter(a => a.severity === 'critical').length > 0 ? 'text-red-600 dark:text-red-400' : 'text-terra-400 dark:text-[#4D8A72]',
            borderColor: alerts.filter(a => a.severity === 'critical').length > 0 ? 'border-red-200/60 dark:border-red-800/30' : 'border-terra-200/60 dark:border-[#1E5A47]/40',
