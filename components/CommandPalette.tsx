                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${
                        globalIndex === selectedIndex
                          ? 'bg-jade-50 dark:bg-jade-900/20 text-jade-700 dark:text-jade-400'
                          : 'text-[var(--text-secondary)] hover:bg-[var(--bg-content)]'
                      }`}
                    >
                      <cmd.icon className={`w-4 h-4 shrink-0 ${globalIndex === selectedIndex ? 'text-jade-500' : 'text-[var(--text-tertiary)]'}`} />
                      <span className="font-medium flex-1 text-left">{cmd.label}</span>
                      {globalIndex === selectedIndex && <ArrowRight className="w-3.5 h-3.5 text-jade-500" />}
                    </button>
                  );
                })}
              </div>
