  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b-4 border-slate-800 dark:border-slate-600 pb-4 transition-colors">
        <div>
          <h2 className="text-3xl font-semibold text-slate-900 dark:text-white font-heading">Resource Toolkit</h2>
          <p className="text-slate-600 dark:text-slate-400 font-semibold text-xs mt-1">Optimize Inputs & Reduce Waste</p>
        </div>
        <div className="mt-4 md:mt-0 flex gap-2">
           <button 
             onClick={() => setMode('FERTILIZER')}
             aria-pressed={mode === 'FERTILIZER'}
             className={`px-4 py-2 rounded font-semibold text-xs border-2 focus:outline-none focus:ring-2 focus:ring-slate-500 transition-colors ${mode === 'FERTILIZER' ? 'bg-slate-900 dark:bg-white text-yellow-500 dark:text-slate-900 border-slate-900 dark:border-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-300 dark:border-slate-600'}`}
           >
             Fertilizer
           </button>
           <button 
             ...
             className={`px-4 py-2 rounded font-semibold text-xs border-2 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors ${mode === 'IRRIGATION' ? 'bg-blue-900 dark:bg-blue-600 text-white border-blue-900 dark:border-blue-600' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-300 dark:border-slate-600'}`}
           >
             Irrigation
           </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded shadow-sm border border-slate-200 dark:border-slate-700 transition-colors">
           ...
           {mode === 'FERTILIZER' ? (
             <div className="space-y-4">
                <div>
                   <label htmlFor="targetN" className="block text-xs font-semibold text-slate-800 dark:text-slate-300 mb-1">Target Nitrogen (N)</label>
                ...
                <div>
                   <label htmlFor="fertType" className="block text-xs font-semibold text-slate-800 dark:text-slate-300 mb-1">Fertilizer Type</label>
                ...
                <div>
                   <label htmlFor="fertArea" className="block text-xs font-semibold text-slate-800 dark:text-slate-300 mb-1">Field Area</label>
                   <div className="flex items-center">
             ...
             <div className="space-y-4">
                ...
                <div>
                   <label htmlFor="et0" className="block text-xs font-semibold text-slate-800 dark:text-slate-300 mb-1">Evapotranspiration (ET0)</label>
                   <div className="flex items-center">
                ...
                <div>
                   <label htmlFor="efficiency" className="block text-xs font-semibold text-slate-800 dark:text-slate-300 mb-1">System Efficiency</label>
                ...
                <div>
                   <label htmlFor="irrArea" className="block text-xs font-semibold text-slate-800 dark:text-slate-300 mb-1">Field Area</label>
                   <div className="flex items-center">
                ...
                <button onClick={calculateIrrigation} className="w-full bg-blue-900 dark:bg-blue-600 text-white py-4 font-semibold hover:bg-blue-800 dark:hover:bg-blue-500 mt-4 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded transition-colors shadow-lg">Calculate Water Needs</button>
             </div>
           )}
        </div>
        ...
        <div className="bg-slate-900 dark:bg-slate-950 text-white p-6 rounded shadow-lg border-l-8 border-yellow-500 flex flex-col justify-center relative overflow-hidden transition-colors" role="status" aria-live="polite">
           ...
           <div className="relative z-10 text-center">
              <h4 className="text-slate-300 font-semibold text-sm mb-4">Calculated Output</h4>
              
              {mode === 'FERTILIZER' ? (
                <>
                  <div className="text-7xl font-bold font-heading text-white mb-2">{fertResult}</div>
                  <div className="text-yellow-500 text-xl font-bold mb-6">Kilograms Needed</div>
                  <div className="bg-slate-800 dark:bg-slate-900 p-4 rounded text-left border border-slate-700">
                    <p className="text-xs text-slate-300 font-semibold mb-2">Cost Estimation</p>
                ...
                <>
                  ...
                  <div className="bg-slate-800 dark:bg-slate-900 p-4 rounded text-left border border-slate-700">
                    <p className="text-xs text-slate-300 font-semibold mb-2">Conservation Tip</p>
