...
const ResourceCalculator: React.FC = () => {
  ...
  const inputCls = "w-full p-3 bg-card-dynamic border border-primary-dynamic rounded font-bold text-primary-dynamic focus:outline-none focus:border-harvest-500 dark:focus:border-harvest-400";
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end organic-divider pb-4 transition-colors">
        ...
        <div className="mt-4 md:mt-0 flex gap-2">
           <button 
             ...
             className={`px-4 py-2 rounded font-semibold text-xs border-2 focus:outline-none focus:ring-2 focus:ring-field-500 transition-colors ${mode === 'FERTILIZER' ? 'bg-field-800 dark:bg-harvest-100 text-harvest-500 dark:text-field-900 border-field-800 dark:border-harvest-200' : 'bg-card-dynamic text-secondary-dynamic border-primary-dynamic'}`}
           >
             Fertilizer
           </button>
           <button 
             ...
             className={`px-4 py-2 rounded font-semibold text-xs border-2 focus:outline-none focus:ring-2 focus:ring-field-500 transition-colors ${mode === 'IRRIGATION' ? 'bg-field-700 dark:bg-field-600 text-white border-field-700 dark:border-field-600' : 'bg-card-dynamic text-secondary-dynamic border-primary-dynamic'}`}
           >
             Irrigation
           </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="card-surface p-6">
           <h3 className="text-xl font-semibold text-primary-dynamic mb-6 flex items-center">
             {mode === 'FERTILIZER' ? <Sprout className="w-6 h-6 mr-2 text-field-600 dark:text-field-400" aria-hidden="true"/> : <Droplets className="w-6 h-6 mr-2 text-field-600 dark:text-field-400" aria-hidden="true"/>}
           ...
           {mode === 'FERTILIZER' ? (
             <div className="space-y-4">
                ...
                <button onClick={calculateFertilizer} className="w-full bg-field-800 dark:bg-harvest-500 text-harvest-500 dark:text-field-900 py-4 font-semibold hover:bg-field-700 dark:hover:bg-harvest-400 mt-4 focus:outline-none focus:ring-2 focus:ring-harvest-500 focus:ring-offset-2 rounded transition-colors shadow-lg">Calculate Requirements</button>
             </div>
           ) : (
             <div className="space-y-4">
                <div>
                ...
                <button onClick={calculateIrrigation} className="w-full bg-field-700 dark:bg-field-600 text-white py-4 font-semibold hover:bg-field-600 dark:hover:bg-field-500 mt-4 focus:outline-none focus:ring-2 focus:ring-field-400 focus:ring-offset-2 rounded transition-colors shadow-lg">Calculate Water Needs</button>
             </div>
           )}
        </div>

        <div className="bg-field-800 dark:bg-field-950 text-white p-6 rounded shadow-lg border-l-8 border-harvest-500 flex flex-col justify-center relative overflow-hidden transition-colors" role="status" aria-live="polite">
           <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-white via-transparent to-transparent"></div>
           
           <div className="relative z-10 text-center">
              <h4 className="text-field-300 font-semibold text-sm mb-4">Calculated Output</h4>
              
              {mode === 'FERTILIZER' ? (
                <>
                  <div className="text-7xl font-bold font-heading text-white mb-2">{fertResult}</div>
                  <div className="text-harvest-500 text-xl font-bold mb-6">Kilograms Needed</div>
                  <div className="card-inner p-4 rounded text-left">
                    <p className="text-xs text-field-300 font-semibold mb-2">Cost Estimation</p>
                ...
                <>
                  <div className="text-7xl font-bold font-heading text-field-300 mb-2">{irrigationResult.toLocaleString()}</div>
                  <div className="text-white text-xl font-bold mb-6">Liters / Day</div>
                  <div className="card-inner p-4 rounded text-left">
                    <p className="text-xs text-field-300 font-semibold mb-2">Conservation Tip</p>
                    <p className="text-sm">Switching to Drip Irrigation (90% Eff) would save <span className="text-field-400 font-bold">{(irrigationResult - (irrigationResult * (efficiency/0.9))).toLocaleString()} Liters</span> daily.</p>
                  </div>
                </>
              )}
           </div>
