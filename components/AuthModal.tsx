       ...
       <div className="w-full max-w-2xl mx-4 rounded-3xl shadow-2xl overflow-hidden border border-[var(--border-card)] animate-fade-in-up bg-[var(--bg-card)] flex flex-col max-h-[90vh] card-surface">
        ...
        <div className="relative px-8 pt-8 pb-4">
          <div className="absolute inset-0 pointer-events-none">
            <div className="w-full h-full bg-[radial-gradient(ellipse_at_top,_var(--bg-jade-50)_0%_,transparent_70%)] dark:bg-[radial-gradient(ellipse_at_top,_rgba(30,58,40,0.3)_0%_,transparent_70%)]"></div>
          </div>
           <div className="flex flex-col items-center text-center py-6">
             <div className="flex items-center gap-2 mb-4">
               <div className="w-12 h-12 bg-gradient-to-br from-jade-500 to-sunburst-500 rounded-xl flex items-center justify-center shadow-lg shadow-jade-500/20">
            ...
            {STEPS.map((s, i) => (
              <React.Fragment key={s.num}>
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
                  step === s.num
                    ? 'bg-jade-600 text-white shadow-md shadow-jade-500/25'
                    : step > s.num
                    ? 'bg-jade-50 dark:bg-jade-900/40 text-jade-700 dark:text-jade-400'
                ...
                {i < STEPS.length - 1 && (
                  <div className={`flex-1 h-0.5 rounded-full transition-all ${
                    step > s.num ? 'bg-jade-400' : 'bg-[var(--border-card)]'
                  }`} />
                )}
              </React.Fragment>
            ))}
        ...
        <div className="flex-1 px-8 pb-6 overflow-y-auto">
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="relative group">
                  <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--text-tertiary)] group-focus-within:text-jade-500 transition-colors" />
                  <input
                    type="text"
                    value={name}
              ...
              <div>
                ...
                <div className="relative group">
                  <Tractor className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--text-tertiary)] group-focus-within:text-jade-500 transition-colors" />
                  <input
                    type="text"
                    value={farmName}
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="relative group">
                <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--text-tertiary)] group-focus-within:text-jade-500 transition-colors" />
                <input
                  type="text"
                  value={countrySearch}
                    ...
                    onClick={() => setSelectedCountry(country)}
                     className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all text-left ${
                       selectedCountry?.code === country.code
                         ...
                         : 'border-[var(--border-card)] hover:border-jade-300 dark:hover:border-jade-700 bg-[var(--bg-content)]'
                     }`}
                  >
                    ...
                    {selectedCountry?.code === country.code && (
                      <Check className="w-5 h-5 text-jade-500 flex-shrink-0" />
                    )}
                  </button>
                ))}
              </div>
          ...
          {step === 3 && selectedCountry && (
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                  {FARM_TYPE_OPTIONS.map(({ value, label, icon: Icon, desc }) => (
                    ...
                    >
                      ...
                      <span className={`text-xs font-bold ${farmType === value ? 'text-jade-700 dark:text-jade-400' : 'text-[var(--text-primary)]'}`}>{label}</span>
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <input
                    ...
                    className="flex-1 accent-jade-500"
                  />
              ...
              <div className="bg-jade-50 dark:bg-jade-900/20 rounded-xl p-4 border border-jade-200 dark:border-jade-800">
                <div className="flex items-center gap-2 mb-2">
                  <MapPin className="w-4 h-4 text-jade-600 dark:text-jade-400" />
                  <span className="text-xs font-bold text-jade-700 dark:text-jade-400">Region defaults</span>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedCrops.map(crop => (
                      <span key={crop.id} className="px-3 py-1.5 bg-jade-50 dark:bg-jade-900/20 text-jade-700 dark:text-jade-400 rounded-full text-xs font-bold border border-jade-200 dark:border-jade-800">
                        {crop.name}
                      </span>
                    ))}
          ...
          {step > 1 ? (
           ...
           {step < 4 ? (
             <button
               ...
               onClick={handleNext}
               disabled={!canNext()}
               className="px-8 py-4 bg-jade-800 dark:bg-sunburst-500 hover:bg-jade-700 dark:hover:bg-sunburst-400 text-white dark:text-jade-950 font-bold text-sm rounded-xl transition-all shadow-lg shadow-jade-500/15 dark:shadow-sunburst-500/15 hover:shadow-jade-500/25 dark:hover:shadow-sunburst-500/25 flex items-center gap-2 active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed"
             >
               Continue
